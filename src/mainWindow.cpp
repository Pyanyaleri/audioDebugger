...
void ADMainWindow::update() {
	...
	bool *enables[] = {&at_instance.enable,		&ao_instance.enable,
					   &camera_instance.enable, &config_instance.enable,
					   &el_instance.enable,		&mo_instance.enable,
	...
	/* Row 1 */ { at_instance.show(childWBase, childHBase, childFlags); }
	...
	if (enable) {
		if (ImGui::BeginMenuBar()) {
			...
			if (ImGui::BeginMenu("View")) {
				...
				ImGui::MenuItem("Audio Timeline", NULL,
								enables[AUDIO_TIMELINE]);
				ImGui::MenuItem("Oscilloscope", NULL,
								enables[AUDIO_OSCILLOSCOPE]);
...
void ADMainWindow::AudioTimeline::show(float sizeW, float sizeH,
	...
	if (enable) {
		...
		if (ImPlot::BeginPlot("##plot_timeline", ImGui::GetContentRegionAvail(),
			...
			ImPlot::PlotShaded("##audio_coords", plotTimes.data(),
							   plotMins.data(), plotMaxs.data(),
							   plotTimes.size());
		}
...
void ADMainWindow::AudioTimeline::loadTestAudio() {
	if (enable) {
		...
		for (int timelineInd = 0; timelineInd < totalAudioSamples;
			 timelineInd++) {
			float tCoord = timestampLength * timelineInd;
			timelineData.push_back(
				ImVec2(tCoord, (sinf(tCoord) + sinf(2 * tCoord)) / 2));
		}

		int plottingRatio = static_cast<int>(totalAudioSamples / TOTAL_SAMPLES);

		for (int plottingStep = 0; plottingStep < totalAudioSamples;
			 plottingStep += plottingRatio) {
			...
			float min = (*std::min_element(chunkStart, chunkEnd,
										   [](ImVec2 &vec1, ImVec2 &vec2) {
											   return vec1.y < vec2.y;
										   }))
							.y;
			float max = (*std::max_element(chunkStart, chunkEnd,
										   [](ImVec2 &vec1, ImVec2 &vec2) {
											   return vec1.y < vec2.y;
										   }))
							.y;
...
void ADMainWindow::AudioOscilloscope::show(float sizeW, float sizeH,
										   ImGuiChildFlags flags) {
	if (enable) {
		...
		if (ImPlot::BeginPlot("##plot_mono_audio",
			...
			for (int i = 0; i < 3; ++i) {
				if (showDigital[i] && dataDigital[i].Data.size() > 0) {
					...
					ImPlot::PlotDigital(
						label, &dataDigital[i].Data[0].x,
						&dataDigital[i].Data[0].y, dataDigital[i].Data.size(),
						{ImPlotProp_Offset, dataDigital[i].Offset,
						 ImPlotProp_Stride, 2 * sizeof(float), ImPlotProp_Size,
						 (i + 1) * 4});
				}
			}
			for (int i = 0; i < 2; ++i) {
...
void ADMainWindow::CameraOutput::show(float sizeW, float sizeH,
									  ImGuiChildFlags flags) {
	if (enable) {
...
void ADMainWindow::Configuration::show(float sizeW, float sizeH,
									   ImGuiChildFlags flags) {
	if (enable) {
		...
		ImGui::InputInt("Detection Delay (ms)",
						&programSettings.detection_delay_ms);
		ImGui::InputFloat("Whistle Confidence Threshold",
						  &programSettings.whistle_confidence_thresh);

		ImGui::EndChild();
	}
}

void ADMainWindow::EventLog::show(float sizeW, float sizeH,
								  ImGuiChildFlags flags) {
	if (enable) {
...
void ADMainWindow::ModelOutput::show(float sizeW, float sizeH,
									 ImGuiChildFlags flags) {
	if (enable) {
...
void ADMainWindow::Spectrogram::show(float sizeW, float sizeH,
									 ImGuiChildFlags flags) {
	if (enable) {
