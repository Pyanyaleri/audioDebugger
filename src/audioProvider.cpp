...
void AudioProvider::loadAudioFile(std::string filename) {
	...
	if (result != MA_SUCCESS) {
		fmt::print(stderr,
				   "File opened does not exist or is not supported...\n");
	}
...
void AudioProvider::playAudio() {
	if (deviceInitialized) {
		...
		fmt::print(stderr,
				   "Device not initialized. Do that before playing audio...\n");
	}
...
void AudioProvider::pauseAudio() {
	if (deviceInitialized) {
		...
		fmt::print(
			stderr,
			"Device not initialized. Do that before stopping audio...\n");
	}

	play = false;
}

void AudioProvider::dataCallback(ma_device *pDev, void *pOut, const void *pIn,
